...
public class PlanServiceImpl implements PlanService {
	...
	public PlanRespDto addPlan(PlanDto plan) {
		if (plan != null) {
			...
			throw new BillingException("Input should not be null");
		}
	}
	...
	public PlanOverdueRespDto addOverdueDetails(PlanOverdueDto plan) {
		if (plan != null) {
			...
			int count = 0;
			try {
				if (plan.getOverageService() != 0 && plan.getServiceCost() != 0.0 && plan.getOverageType() != null
						&& plan.getUnit() != null && plan.getPlan() != null) {
					count++;
			...
			try {
				...
				if (plan.getOverageType() == null) {
					...
					if (count == 0) {
						...
						throw new BillingException(
								"Failed to update plan overdue. Check with overage type and plan ID");
					}
				}
			}
		} else {
